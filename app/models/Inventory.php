...
class Inventory{
    ...
    public function listProducts(){
        try{
            $sql = "Select * from product p inner join productforsale p2 on p.id_product = p2.id_product inner join categoryp c on p.id_categoryp = c.id_categoryp order by p.product_name";
        ...
        try {
            if(empty($model->id_product)){
                $sql = 'insert into product(
                    id_categoryp, product_barcode, product_name, product_description, product_unid_type, product_stock, product_created_at
                    ) values(?,?,?,?,?,?,?)';
                $stm = $this->pdo->prepare($sql);
                $stm->execute([
                    $model->id_categoryp,
